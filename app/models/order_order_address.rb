class OrderOrderAddress
  ...
  validates :number, format: { with: /\A\d{10,11}\z/}, allow_blank: true

  def save
    order = Order.create(user_id: user_id, item_id: item_id)
    OrderAddress.create(post_code: post_code, pref_id: pref_id, city: city, address_line1: address_line1,
                        address_line2: address_line2, number: number, order_id: order.id)
  end
end
